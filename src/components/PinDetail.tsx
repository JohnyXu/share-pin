...
function PinDetail({ user }: PinDetailProps) {
  ...
  const pinIdString: string = pinId as string;
  const [pins, setPins] = useState<IPin[]>([]);
  const [pinDetail, setPinDetail] = useState<null | IPin>(null);
  const [comment, setComment] = useState('');
  const [addingComment, setAddingComment] = useState(false);

  const fetchPinDetails = (pidString: string) => {
    const query = pinDetailQuery(pidString);
          ...
          <div className="w-full p-5 flex-1 xl:min-w-620">
            <div className="flex items-center justify-between">
              <div className="flex gap-2 items-center">
                <a
                  href={`${getSanityImageUrl(pinDetail.image)}?dl=`}
                  download
